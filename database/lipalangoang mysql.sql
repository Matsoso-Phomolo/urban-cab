...
-- ── 7. Payments ───────────────────────────────────────────────
-- One payment record per ride keeps payment tracking simple and explainable.
CREATE TABLE Payments (
  ...
  received_by_driver_id INT                   DEFAULT NULL,
  PRIMARY KEY (payment_id),
  UNIQUE KEY uq_payment_ride (ride_id),
  CONSTRAINT chk_payment_status
    CHECK (payment_status IN ('PENDING','COMPLETED','CANCELLED')),
  FOREIGN KEY fk_payment_ride   (ride_id)               REFERENCES Rides(ride_id),
  FOREIGN KEY fk_payment_method (payment_method_id)     REFERENCES Payment_Methods(payment_method_id),
  FOREIGN KEY fk_payment_driver (received_by_driver_id) REFERENCES Drivers(driver_id),
...
CREATE TABLE Ride_Ratings (
  ...
  UNIQUE KEY uq_rating_ride (ride_id),
  CONSTRAINT chk_rating_score
    CHECK (rating_score BETWEEN 1 AND 5),
  FOREIGN KEY fk_rating_ride (ride_id) REFERENCES Rides(ride_id)
) ENGINE=InnoDB;
...
VALUES
  (1, 1, 'COMPLETED', 80.00,  80.00, '2025-01-10 08:22:00', 1),
  (2, 2, 'COMPLETED', 80.00,  80.00, '2025-01-10 09:35:00', 2),
  (3, 1, 'COMPLETED', 80.00,  80.00, '2025-01-11 08:00:00', 1),
  (4, 1, 'PENDING',  120.00,   NULL, NULL,                  NULL);

-- ── Sample ride ratings ───────────────────────────────────────
INSERT INTO Ride_Ratings (ride_id, rating_score, rating_comment) VALUES
  (1, 5, 'Driver arrived quickly and the trip was smooth.'),
  (2, 4, 'Good ride overall.');

...
app.config['MYSQL_PASSWORD'] = os.environ.get('DB_PASSWORD', 'your_mysql_password')
...
def login():
    if request.method == 'POST':
        ...
        if role == 'driver':
            u = q("SELECT * FROM Drivers WHERE phone_number=?", (ident,), one=True)
            ...
            u = q("SELECT * FROM Admin_Users WHERE username=? OR email=?", (ident,ident), one=True)
            if u and check_pw(pw, u['password_hash']):
...
def track(ride_id):
    ride = q("""SELECT r.*,
        ...
        WHERE r.ride_id=?""", (ride_id,), one=True)
    if not ride:
...
def api_status(ride_id):
    ride = q("SELECT ride_status, driver_id FROM Rides WHERE ride_id=?", (ride_id,), one=True)
...
@auth('driver')
def driver_home():
    driver   = q("SELECT * FROM Drivers WHERE driver_id=?", (session['uid'],), one=True)
    pending  = q("""SELECT r.*,
    ...
    my_rides = q("""SELECT r.*,
        ...
        JOIN Locations dr ON r.dropoff_location_id=dr.location_id
        JOIN Payment_Methods pm ON r.payment_method_id=pm.payment_method_id
        WHERE r.driver_id=? AND r.ride_status IN ('ACCEPTED','IN_PROGRESS')
...
def driver_accept(ride_id):
    ride = q("SELECT * FROM Rides WHERE ride_id=? AND ride_status='REQUESTED'", (ride_id,), one=True)
    if ride:
        m("UPDATE Rides SET driver_id=?,ride_status='ACCEPTED',accepted_at=NOW() WHERE ride_id=?",
          (session['uid'], ride_id))
        m("UPDATE Drivers SET is_available=0 WHERE driver_id=?", (session['uid'],))
...
def driver_start(ride_id):
    m("UPDATE Rides SET ride_status='IN_PROGRESS' WHERE ride_id=? AND driver_id=? AND ride_status='ACCEPTED'",
      (ride_id, session['uid']))
...
def driver_complete(ride_id):
    ...
    ride = q("SELECT fare_amount FROM Rides WHERE ride_id=? AND driver_id=? AND ride_status='IN_PROGRESS'",
    ...
    m("UPDATE Rides SET ride_status='COMPLETED',completed_at=NOW() "
      "WHERE ride_id=? AND driver_id=? AND ride_status='IN_PROGRESS'",
      (ride_id, session['uid']))
    m("UPDATE Drivers SET is_available=1 WHERE driver_id=?", (session['uid'],))
...
@auth('driver')
def driver_toggle():
    d   = q("SELECT is_available FROM Drivers WHERE driver_id=?", (session['uid'],), one=True)
    new = 0 if d['is_available'] else 1
    m("UPDATE Drivers SET is_available=? WHERE driver_id=?", (new, session['uid']))
...
def admin_home():
    ...
    recent = q("""SELECT r.*,
        pu.location_name AS pickup_name, dr.location_name AS dropoff_name,
        d.first_name||' '||d.last_name AS driver_name, pm.method_name
        FROM Rides r
        JOIN Locations pu ON r.pickup_location_id=pu.location_id
        JOIN Locations dr ON r.dropoff_location_id=dr.location_id
        JOIN Payment_Methods pm ON r.payment_method_id=pm.payment_method_id
...
def admin_rides():
    ...
    sql  = """SELECT r.*,
        ...
        JOIN Locations dr ON r.dropoff_location_id=dr.location_id
        JOIN Payment_Methods pm ON r.payment_method_id=pm.payment_method_id
        LEFT JOIN Drivers d ON r.driver_id=d.driver_id WHERE 1=1"""
    args = []
...
def admin_cancel_ride(ride_id):
    ride = q("SELECT * FROM Rides WHERE ride_id=?", (ride_id,), one=True)
    if ride and ride['ride_status'] in ('REQUESTED','ACCEPTED','IN_PROGRESS'):
        m("UPDATE Rides SET ride_status='CANCELLED' WHERE ride_id=?", (ride_id,))
        if ride['driver_id']:
            m("UPDATE Drivers SET is_available=1 WHERE driver_id=?", (ride['driver_id'],))
...
def admin_drivers():
    ...
    sql  = """SELECT d.*, COUNT(r.ride_id) as total_rides,
        ...
        WHERE 1=1"""
    args = []
    if search: sql += " AND (d.first_name||' '||d.last_name LIKE %s OR d.vehicle_plate LIKE %s OR d.license_number LIKE %s)"; args += [f'%{search}%']*3
...
def admin_edit_location(lid):
    loc = q("""SELECT l.*,
        (SELECT COUNT(*) FROM Rides WHERE pickup_location_id=l.location_id OR dropoff_location_id=l.location_id) as ride_count
        FROM Locations l WHERE l.location_id=?""", (lid,), one=True)
    if not loc:
